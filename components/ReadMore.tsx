...
export default function ReadMore({ articles }: ReadMoreProps) {
  ...
  return (
    <div className="max-w-[760px] mx-auto px-4 sm:px-6 lg:px-8 py-8 border-t border-gray-200 mt-6">
      <h2 className="text-[15px] font-bold font-sans uppercase tracking-[0.05em] text-black mb-7">
        READ MORE
      </h2>
      <div className="flex flex-col space-y-7">
        {articles.slice(0, 4).map((article, i) => {
          ...
          if (i < 2 && !displayDate.includes('UPDATED')) {
             displayDate = `UPDATED: ${displayDate}, 9:30 P.M. CENTRAL`;
          }
            ...
            <article key={article.id || i} className="flex flex-row gap-5 group cursor-pointer">
              ...
              <div className="flex flex-col w-[62%] sm:w-[68%] pt-0.5">
                <Link href={`/${article.category.toLowerCase().replace(/\s+/g, '-')}/${article.slug}`} className="block">
                  <h3 className="text-[17px] sm:text-[21px] font-bold font-[Georgia,serif] leading-[1.2] text-black group-hover:text-gray-700 transition-colors mb-1.5">
                    {article.headline}
                  </h3>
                </Link>
